...
- User interface
    ...
    - Animations within annotation views
        ...
        - Each annotation needs an animation start and end time within the scope of the whole timeline.
        - When the local animation time is in bounds, adjust the ripple layer scale and opacity.
...
- Memory, performance, battery, GPS usage testing
    - Memory never seems to climb above 16MB, and I think the majority of that is MapKit.
    - iPhone 4S saw a batch of leaked CALayers, the ripples I'm animating on the map annotations. But the reusable annotation views shouldn't ever be destroyed, so I'm not sure why the layers would ever lose their incoming references.
    - In object mapping I was converting plist NSDictionaries to native swift [String: AnyObject], and NSArrays to [AnyObject], but that was the vast majority of object mapping time. Leaving the objects as NSArrays and NSDictionaries was about 10x faster and worked with no other changes to code. A query now parses in about 30-50ms instead of 300-500ms. I'm still glad that's on a background thread though.
    - iPhone 4S gets 60fps during display link animation. Panning the map while animating skips a little bit, but no more than you'd expect for MapKit on the 4S. The hybrid map is probably the slowest, but its appearance is the best for this app.
    - I tried speeding up display link animation for the sake of battery, but no changes made meaningful improvements. MapKit is spending the bulk of the energy.
    - The GPS chip never activates, as intended.
- One day I may write unit tests for this app… but it is not this day.
